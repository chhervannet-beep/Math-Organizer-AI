...
// Helper to make Gemini API requests robust against transient errors (like 503, 429, 500)
async function generateContentWithRetry(aiClient: GoogleGenAI, params: any, maxRetries = 3) {
  ...
  for (const model of modelsToTry) {
    ...
    while (attempt < maxRetries) {
      try {
        console.log(`[Gemini - Vercel Serverless] Calling API model "${model}" - Attempt ${attempt + 1}/${maxRetries}...`);
        ...
        console.warn(`[Gemini - Vercel Serverless] Attempt ${attempt} with model "${model}" failed/errored:`, errMsg);
        
        if (attempt >= maxRetries) {
          console.warn(`[Gemini - Vercel Serverless] Model "${model}" retries exhausted. Moving to next fallback if available.`);
          break;
        }
        ...
        if (isRetriable) {
          console.log(`[Gemini - Vercel Serverless] Retriable error. Backing off for ${delay}ms...`);
  ...
  throw lastError || new Error("Gemini API calls failed on all attempted models after retries.");
}
...
Optional customization from user: ${customPrompt || "None"}.`;
    ...
    const documentSchema = {
      type: Type.OBJECT,
      properties: {
        title: { type: Type.STRING, description: "Elegant title of the document in Khmer (e.g. វិញ្ញាសาគណិតវិទ្យាត្រៀមប្រលងបាក់ឌុប or មេរៀនសង្ខេបៈ ចំនួនកុំផ្លិច)" },
    ...
    console.error("Error organizing math with Gemini on Vercel:", error);
    return res.status(500).json({
